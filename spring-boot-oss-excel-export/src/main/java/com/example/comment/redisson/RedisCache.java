...
public class RedisCache {
    ...
    public <T> boolean trySet(String key, T value, Codec codec) {
        RBucket<T> bucket = redisson.getBucket(key, codec);
        return bucket.trySet(value, DEFAULT_EXPIRED, TimeUnit.SECONDS);
    }

    public <T> boolean trySet(String key, T value, long expired, Codec codec) {
        RBucket<T> bucket = redisson.getBucket(key, codec);
        return bucket.trySet(value, expired, TimeUnit.SECONDS);
    }
